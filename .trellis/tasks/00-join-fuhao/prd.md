...
`fuhao` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/fuhao/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/fuhao/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee fuhao` to
...
python3 ./.trellis/scripts/task.py archive 00-join-fuhao
```
